...
const sendMessage = async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Message received and decrypted successfully.' });
    } catch (error) {
        console.error('Error:', error);
        res.status(500).json({ success: false, error: 'Failed to decrypt message.' });
    }
};
...
const sendPasswordReset = async (req, res) => {
    ...
    try{
        nodemailer.forgotPassword(email)
